        ...
        <div class="lg:flex relative">
            <div class="sidebar bg-blue-900 w-full lg:w-1/5 lg:h-screen text-center rounded flex lg:flex-col gap-2 lg:gap-7 lg:flex-wrap justify-around lg:justify-start items-center">
                ...
                <div class="icon flex gap-5 justify-center">
                        ...
                        <p class="my-1 lg:my-3 font-bold text-yellow-100"><a href="../Clients/afficherClients.php">Clients</a></p>
                </div>
                <div class="icon flex gap-5 justify-center">
            ...
            <div class="w-full min-h-[90vh] bg-FormBg bg-cover justify-center items-center bg-center bg-no-repeat" style="background-image: url('../../imgs/c.jpg');">
                <?php
                    if ($successMessage){
                        echo "
                            <div class='w-1/3 mt-10 py-6 ps-5 bg-green-500 rounded-md'>
                                <p class='text-white font-bold'>$successMessage</p>
                            </div>
                        ";
                    }
                ?>
                <div class=" ms-5">
                    <button class="bg-blue-900 px-5 mt-5 py-3 text-yellow-100 rounded"><a href="./ajouterContrat.php">Ajouter Contrat</a></button>
                </div>
                <div class="clients w-full top-4 px-3">
                    <p class="text-center pb-2 font-bold text-blue-900 text-2xl">Liste des contrats</p>
                    <table class="table-auto border-collapse border border-gray-500  w-full">
                        <tr>
                            <th class=" py-2 px-2 text-left text-sm lg:text-[15px] bg-blue-300">Num Contrat</th>
                            <th class=" py-2 px-4 text-left text-[12px] lg:text-[15px] bg-blue-300">Date début</th>
                            <th class=" py-2 px-2 text-left text-[12px] lg:text-[15px] bg-blue-300">Date Fin</th>
                            <th class=" py-2 px-2 text-left text-[12px] lg:text-[15px] bg-blue-300">Durée</th>
                            <th class=" py-2 px-2 text-left text-[12px] lg:text-[15px] bg-blue-300">Num Client</th>
                            <th class=" py-2 px-3 lg:px-1 text-left text-[12px] lg:text-[15px] bg-blue-300">Num Immatriculation</th>
                            <th class=" py-2 px-4 text-left text-[12px] lg:text-[15px] bg-blue-300">Action</th>
                        </tr>
                        <?php
                            if ($result->num_rows > 0) {
                                while($row = $result->fetch_assoc()) {
                                    echo "<tr class='even:bg-yellow-50 odd:bg-blue-300'>";
                                    echo "<td class='px-2 py-2  lg:px-7 text-[12px] lg:text-[15px]'>" . $row['NumContrat'] . "</td>";
                                    echo "<td class='px-3 py-2 text-[12px] lg:text-[15px]'>" . $row['DateDebut'] . "</td>";
                                    echo "<td class='px-3 py-2 text-[12px] lg:text-[15px]'>" . $row['DateFin'] . "</td>";
                                    echo "<td class='px-2 py-2 text-[12px] lg:text-[15px]'>" . $row['Duree'] . "</td>";
                                    echo "<td class='px-2 py-2 text-[12px] lg:text-[15px]'>" . $row['num_client'] . "</td>";
                                    echo "<td class='px-3 py-2 text-[12px] lg:text-[15px]'>" . $row['NumImmatriculation'] . "</td>";
                                    echo "<td class='flex gap-2 px-4 py-2'>
                                    <a href='../../Controllers/Contrats/getContratInfo.php?id=" . $row['NumContrat'] . "'>
                                    ...
                                    <a href='../../Controllers/Contrats/deleteContrat.php?id=" . $row['NumContrat'] . "'>
