<?php
    ...
    $voiture_infos = $_SESSION['voiture_infos'] ?? null;
    unset($_SESSION['marqueErr'],$_SESSION['modeleErr'],$_SESSION['anneeErr']);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="sidebar bg-blue-900 w-full lg:w-1/5 lg:h-screen text-center rounded flex lg:flex-col gap-2 lg:gap-7 lg:flex-wrap justify-around lg:justify-start items-center">
            ...
            <div class="w-full min-h-[90vh] bg-FormBg bg-cover flex justify-center items-center bg-center bg-no-repeat" style="background-image: url('../../imgs/c.jpg');">
        <fieldset class="flex justify-center items-center w-4/5">
            <div class="w-[90%] lg:w-[50%]">
                <p class="bg-yellow-100 mt-5 rounded text-center text-xl font-bold py-2 text-blue-900">Modifier Voiture</p>
                <form class="flex flex-col justify-center pl-10 bg-blue-900 h-96 rounded-md shadow-xl" action="../../Controllers/Voitures/updateVoiture.php" method="post">
                    <label class="text-yellow-100">Numéro Immatriculation :</label>
                    <input class="w-[80%] bg-yellow-50 rounded focus:outline-none focus:border-transparent" type="text" name="NumImmatriculation" value="<?php if($voiture_infos) echo $voiture_infos['NumImmatriculation']; ?>"><br>
                    <label class="text-yellow-100">Marque :</label>
                    <input class="w-[80%] bg-yellow-50 rounded focus:outline-none focus:border-transparent" type="text" name="Marque" value="<?php if($voiture_infos) echo $voiture_infos['Marque']; ?>"><br>
                    <?php if ($marqueErr) echo "<p class='text-red-600'>$marqueErr</p><br>"?>
                    <label class="text-yellow-100">Modele :</label>
                    <input class="w-[80%] bg-yellow-50 rounded focus:outline-none focus:border-transparent" type="text" name="Modele" value="<?php if($voiture_infos) echo $voiture_infos['Modele']; ?>"><br>
                    <?php if ($modeleErr) echo "<p class='text-red-600'>$modeleErr</p><br>"?>
                    <label class="text-yellow-100">Année :</label>
                    <input class="w-[80%] bg-yellow-50 rounded focus:outline-none focus:border-transparent" type="text" name="Annee" value="<?php if($voiture_infos) echo $voiture_infos['Annee']; ?>"><br>
