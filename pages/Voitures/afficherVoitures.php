...
<body>
    <div class="whole-container">
        ...
        <div class="sidebar bg-blue-900 w-full lg:w-1/5 lg:h-screen text-center rounded flex lg:flex-col gap-2 lg:gap-7 lg:flex-wrap justify-around lg:justify-start items-center">
            ...
            <div class="w-full min-h-[90vh] bg-FormBg bg-cover bg-center bg-no-repeat" style="background-image: url('../../imgs/c.jpg');">
                <?php
                    if ($successMessage){
                        echo "
                ...
                ?>
                <div class=" ms-5">
                    <button class="bg-blue-900 px-5 py-3 mt-5 text-yellow-100 rounded"><a href="./ajouterVoiture.php">Ajouter Voiture</a></button>
                </div>
                <div class="clients w-full pr-5 top-4 ps-5">
                    <p class="text-center pb-2 font-bold text-blue-900 text-xl lg:text-2xl">Liste des voitures</p>
                    <table class="table-auto border-collapse border border-gray-500 w-full">
                        <tr>
                            <th class=" py-2 px-2 text-left text-sm lg:text-[15px] bg-blue-300">Num Immatriculation</th>
                            <th class=" py-2 px-5 text-left text-sm lg:text-[15px] bg-blue-300">Marque</th>
                            <th class=" py-2 px-2 text-left text-sm lg:text-[15px] bg-blue-300">Modele</th>
                            <th class=" py-2 px-4 text-left text-sm lg:text-[15px] bg-blue-300">Année</th>
                            <th class=" py-2 px-4 text-left text-sm lg:text-[15px] bg-blue-300">Action</th>
                        </tr>
                        <?php
                            if ($result->num_rows > 0) {
                                while($row = $result->fetch_assoc()) {
                                    echo "<tr class='even:bg-yellow-50 odd:bg-blue-300'>";
                                    echo "<td class='px-4 py-2 text-sm lg:text-[15px]'>" . $row['NumImmatriculation'] . "</td>";
                                    echo "<td class='px-4 py-2 text-sm lg:text-[15px]'>" . $row['Marque'] . "</td>";
                                    echo "<td class='px-4 py-2 text-sm lg:text-[15px]'>" . $row['Modele'] . "</td>";
                                    echo "<td class='px-4 py-2 text-sm lg:text-[15px]'>" . $row['Annee'] . "</td>";
                                    echo "<td class='flex gap-2 px-4 py-2'>
                                    <a href='../../Controllers/Voitures/getVoitureInfo.php?id=" . $row['NumImmatriculation'] . "'>
                                    ...
                                    <a href='../../Controllers/Voitures/deleteVoiture.php?id=" . $row['NumImmatriculation'] . "'>
                                        <svg class='text-blue-900 h-5' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 448 512'>
